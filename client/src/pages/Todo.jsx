...
const Todo = () => {
    ...
    const deleteTodo = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:5001/api/todos/${id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
            setTodos(response.data);
        } catch (err) {
    ...
    return (
        <div className="max-w-lg mx-auto p-6 bg-gray-900 text-white rounded-lg shadow-md mt-10">
            ...
            <div className="space-y-4">
                {todos.map((todo) => (
                    <div key={todo._id} className="flex items-center justify-between bg-gray-800 p-3 rounded-md shadow">
                        {editTodoId === todo._id ? (
